...
# Token: This is what everything consists of in programming. For example, 1 + 1 consists of an integer, a plus sign,
# and another integer. The integers and plus signs are tokens. The integers both have values, which are ones. The plus
# sign obviously does not have a value.
...
# Lexer: A lexer is something that performs lexical analysis. In our case it means going over the code and finding out
# what token every word / character has.
...
class Parser:
    ...
    def term(self):
        return self.bin_op(self.factor, (TT_MUL, TT_DIV, TT_MOD, TT_DFL))
    ...
    def dict_expr(self):
        ...
        if self.current_token.type == TT_RCURLY:
            ...
            res.register_advancement()
            self.advance()
        return res.success(DictNode(element_nodes, pos_start, self.current_token.pos_end.copy()))
    ...
    def class_node(self):
        ...
        return res.success(ClassNode(class_name_token, body, pos_start, self.current_token.pos_end))
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_run(self, exec_ctx):
        ...
        _, error, should_exit = run(fn, script)
...
class Interpreter:
    ...
    def visit_DictNode(self, node, context):
        ...
        try:
            for key, value in zip(keys, values):
                elements[res.register(self.visit(key, context))] = res.register(self.visit(value, context))
                if res.should_return():
                    return res
        except ValueError:
            elements[res.register(self.visit(keys[0], context))] = res.register(self.visit(values[0], context))
    ...
    def visit_BinOpNode(self, node, context):  # sourcery no-metrics
        res = RTResult()
        ...
        elif node.op_token.type == TT_DFL:
