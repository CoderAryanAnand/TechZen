...
class TestParser(unittest.TestCase):
    ...
    def test_full_expression(self):
        node = (
            Parser(Lexer("<stdin>", "27 + (43 / 36 - 38) * 51").make_tokens()[0])
            .parse()
            .node
        )
        self.assertEqual(
            str(node.element_nodes),
    ...
    def test_if_statement(self):
        ...
        self.assertEqual(
            str(node.element_nodes[0])[:33], "<TechZen.nodes_.IfNode object at "
        )

    def test_for_loop(self):
        node = (
            Parser(
                Lexer("<stdin>", "for i = 0 to 5 then\nprint(i)\nend").make_tokens()[0]
            )
            .parse()
            .node
        )
        self.assertEqual(
            str(node.element_nodes[0])[:34], "<TechZen.nodes_.ForNode object at "
        )
    ...
    def test_while_loop(self):
        ...
        self.assertEqual(
            str(node.element_nodes[0])[:36], "<TechZen.nodes_.WhileNode object at "
        )

    def test_func_def(self):
        node = (
            Parser(Lexer("<stdin>", "fun add(a,b) -> a + b").make_tokens()[0])
            .parse()
            .node
        )
        self.assertEqual(
            str(node.element_nodes[0])[:38], "<TechZen.nodes_.FuncDefNode object at "
        )
    ...
    def test_try_expr(self):
        node = (
            Parser(
                Lexer(
                    "<stdin>", "try; print('a'); except; print('b'); end"
                ).make_tokens()[0]
            )
            .parse()
            .node
        ...
        self.assertEqual(
            str(node.element_nodes[0])[:34], "<TechZen.nodes_.TryNode object at "
        )
    ...
    def test_class_def(self):
        ...
        self.assertEqual(
            str(node.element_nodes[0])[:36], "<TechZen.nodes_.ClassNode object at "
        )

    def test_list_expr(self):
        node = Parser(Lexer("<stdin>", "[]").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:35], "<TechZen.nodes_.ListNode object at "
        )

    def test_dict_expr(self):
        node = Parser(Lexer("<stdin>", "{}").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:35], "<TechZen.nodes_.DictNode object at "
        )

    def test_continue(self):
        node = Parser(Lexer("<stdin>", "continue").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:39], "<TechZen.nodes_.ContinueNode object at "
        )

    def test_break(self):
        node = Parser(Lexer("<stdin>", "break").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:36], "<TechZen.nodes_.BreakNode object at "
        )

    def test_return(self):
        node = Parser(Lexer("<stdin>", "return").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:37], "<TechZen.nodes_.ReturnNode object at "
        )

    def test_call(self):
        node = Parser(Lexer("<stdin>", "print()").make_tokens()[0]).parse().node
        self.assertEqual(
            str(node.element_nodes[0])[:35], "<TechZen.nodes_.CallNode object at "
        )
